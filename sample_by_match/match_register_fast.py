...
def filter_grasps_fast(
    ...
    for i, g in enumerate(grasps_mod):
        ...
        # 预先计算该 grasp 可能使用到的最大宽度，用一次 crop 覆盖 L1/L2/L2.5/L3
        w_crop = width0
        trials = []
        if enable_widen_after_layer2 and widen_after_layer2 > 1e-9:
            w1 = min(width0 + float(widen_after_layer2), max_aperture)
            w2 = min(width0 + 2.0 * float(widen_after_layer2), max_aperture)
            if w1 > width0 + 1e-6:
                trials.append(w1)
            if w2 > (trials[-1] if trials else width0) + 1e-6:
                trials.append(w2)
            if trials:
                w_crop = max(trials)
        ...
        # 单次近邻裁剪 + 单次坐标变换
        if use_radius_crop:
            idx_near = crop_indices_near_gripper(tree, pos, w_crop, fw, depth, H, palm_t, extra=eps)
            if idx_near.size == 0:
                continue
        else:
            idx_near = np.arange(pts_world.shape[0], dtype=int)

        idx_full = np.asarray(idx_near, dtype=int)
        P_local = world_to_local(pts_world[idx_near], pos, R)

        # -------- Layer-1：无穿透（实体体积）--------
        T0 = time.time()
        if not collision_free_gripper_volume(P_local, width0, hand_params, penetration_slack=penetration_slack):
            continue
        keep1.append(i)
        T1 = time.time()
        # print(f'T1:{T1-T0}')
        # -------- Layer-2：夹缝内有足够点（只计数，不构造 mask）--------
        if not has_points_between_fingers_count(
            P_local, width0, hand_params,
            min_points=min_points_between,
            margin_y=margin_y_between,
            x_clearance_front=x_clearance_front,
            x_clearance_back=x_clearance_back
        ...
        # -------- Layer-2.5：尝试增大 width 再碰撞（复用同一批 P_local）--------
        final_width = width0
        if trials:
            for w_try in trials:
                ok = collision_free_gripper_volume(
        ...
        # -------- Layer-3：最大d + 法线角度（复用同一批 P_local / idx_full）--------
        pass3, dmax, ends = best_span_and_normal_projection_check_nosort(
            P_local=P_local,
            idx_full=idx_full,
            normals_world=normals_world,
            approach_world=approach,
...
def pmatch(path,plane):
    ...
    while True:
        ...
        # print(len(all_for_filter),len(grasps1))
        # 2) width_orig_external 若存在，也要同步裁剪
        if w0_external is not None:
            width1 = np.asarray(w0_external).reshape(-1)[idx_map]
        else:
            width1 = None

        final_grasps, kept_ids, stage_tag, keep3_grasps, keep3_ids, final_w0, keep3_w0 = filter_grasps_fast(
    ...
    # output_grasp = np.tile(output_grasp, (3, 1))
...
if __name__ == "__main__":
    ...
    ply_p = '/home/ubuntu/task/more_than_grasp/assets/YCB_obj/000/nontextured.ply'
